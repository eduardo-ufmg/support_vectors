...
def test_support_vectors():
    ...
    for i, graph_method in enumerate(graph_methods):
        for j, (filter_method, one_step_criterion) in enumerate(
            zip(filter_methods, one_step_criteria)
        ):
            ax = axes[i, j]
            ...
            try:
                ...
                if len(X_support) > 0:
                    ax.scatter(
                        X_support[:, 0], X_support[:, 1], c=y_support, marker="x", s=100
                    )

                ax.set_title(title)

                print(
                    f"{title.replace(chr(10), ' ')}: {len(X_support)} support vectors"
                )
...
def test_individual_methods():
    ...
    try:
        vertices, labels, degrees = get_interclass_vertices(
            X_simple, adj_gabriel, y_simple
        )
    ...
    try:
        X_filtered, y_filtered = filter_by_degree(
            vertices, labels, degrees, "class-average"
        )
    ...
    try:
        X_filtered, y_filtered = filter_by_degree(
            vertices, labels, degrees, "interclass-average"
        )
        print(
            f"  After interclass-average degree filtering: {len(X_filtered)} remaining"
        )
    except Exception as e:
